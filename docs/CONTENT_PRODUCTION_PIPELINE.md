...
> 첨부한 애드센스팜 GPTS/Gems 운영 가이드 중 **가이드 원고 표준 공정에 직접 적용되는 것만** 반영.  
> 쿠팡/쇼핑전환·여행블로그·실버채널·감성 유튜브 대본 류는 제외(`docs/MULTICHANNEL_EXPANSION_PLAN.md` §제외 대상).

### 황금 키워드 GPT/Gem 산출물의 입구
- `docs/SEO_CONTENT_PLAN.md` 의 P0 클러스터 한 행 = 본 12단계 표준 공정의 입력값.
- 단계 1 (키워드 선정) ~ 단계 4 (1분 계산 예시) 까지가 키워드 GPT 흐름과 직결.

### 구글/네이버 SEO 글쓰기 GPT/Gem 의 슬롯
- 단계 2 (제목) → SEO title 슬롯
- 단계 3 (요약 답변) → introSummary 슬롯
- 단계 7 (왜 이 금액이 나왔나요?) → Sejong 번역 본문
- 단계 8 (FAQ) → JSON-LD `FAQPage` 슬롯
- 위 슬롯들은 Kukmincalc **계산기·도구형 사이트** 톤(결론형 + 숫자 + 출처)으로 고정. 일반 블로그 어조 ❌.

### 가이드 페이지 ≠ 일반 블로그
- 가이드 페이지는 **계산기 CTA 3회 이상**이 의무 (단계 5).
- 본문 길이 자체보다 **계산 → 이해 → 다음 행동** 흐름이 우선.
- 가이드 페이지에서 GPT/Gem 출력을 그대로 붙여넣지 않음. 단계 12 검수 체크리스트 통과 필수.

### Phase 3-1 에서의 위치
- 본 파이프라인은 **정의만**. GPT/Gem 호출은 가이드 페이지 발행(3-2) 시 수동.
- 자동화는 Phase 4 이후 (충분한 가이드 누적 후 결정).
